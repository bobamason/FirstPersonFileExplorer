...
public class BaseWorld<T extends BaseEntity> implements Disposable {
    ...
    public T add(final String type, final float x, final float y, final float z){
    ...
    public T add(final String type, final Vector3 position){
        final T entity = constructors.get(type).construct(position.x, position.y, position.z);
        add(entity);
        return entity;
    }
